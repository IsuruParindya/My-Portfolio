...
export default function ProjectCard({
  ...
  return (
    <Card className="group hover-elevate transition-all duration-300" data-testid={`card-project-${title.toLowerCase().replace(/\s+/g, '-')}`}>
      {/* Project Image */}
      <div className="aspect-video bg-gradient-to-br from-primary/20 to-accent/20 rounded-t-lg overflow-hidden">
        {imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="text-6xl font-bold text-primary/30" data-testid={`text-project-placeholder-${title.toLowerCase().replace(/\s+/g, '-')}`}>
              {title.charAt(0)}
            </div>
          </div>
        )}
      ...
      <CardHeader>
        <CardTitle className="text-xl" data-testid={`text-title-${title.toLowerCase().replace(/\s+/g, '-')}`}>
          {title}
        </CardTitle>
